...
import { ClearCreateBuyerError, ClearCreateBuyerSuccess, ClearCreateContactError, ClearCreateContactSuccess, ClearCreateVendorError, ClearCreateVendorSuccess, CreateBuyerFail, CreateBuyerRequest, CreateBuyerSuccess, CreateContactFail, CreateContactRequest, CreateContactSuccess, CreateVendorFail, CreateVendorRequest, CreateVendorSuccess, GetOrganizationError, GetOrganizationRequest, GetOrganizationSuccess } from '@/app/redux/reducers/businessReducer'
import Axios from '../../Axios'
import { buyer, contact, venodor } from './form/organizationform'
import { buyer as Buyer, vendor as Vendor, contact as Contact } from '@/app/redux/interfaces/businessInterface'
...
const ProductDetails = ({props, setTab, tab}:any) => {
  ...
  const {buyerLoading, buyerError, buyerSuccess, vendorLoading, vendorSuccess, vendorError, contactLoading, contactSuccess, contactError, organization} = useSelector((state:RootState)=>state.organization)
  ...
  useEffect(()=>{
    if(buyerSuccess){
      toast.success(buyerSuccess)
    }
    if(buyerError){
      toast.error(buyerError)
    }

    if(vendorSuccess){
      toast.success(vendorSuccess)
    }
    if(vendorError){
    ...
    getOrganization()
  },[buyerSuccess, buyerError, vendorSuccess, vendorError, contactSuccess, contactError, organization])
          ...
          <div>
              <div>
                <div className="collapse bg-base-100 border border-base-300">
                  <input type="checkbox" name="my-accordion-3" checked={firstCheck} onChange={()=>setFirstCheck(true)}/>
                  <div className="collapse-title font-semibold">Organization Create  </div>
                  <div className="collapse-content text-sm flex justify-between">
                      <form onSubmit={bhandleSubmit(handleCreateBuyer)} className='w-3/12 '>
                        <p className='text-sm font-bold text-black text-left'>BUYER CREATE</p>
                        ...
                        <div className="fieldset w-11/12 mt-4">
                          <button className="button w-full py-3 cursor-pointer bg-amber-100 text-black text-xs rounded-sm font-bold">{buyerLoading? <span className="loading loading-spinner loading-xs"></span>:"SUBMIT"}</button>
                        </div>
                        </div>
                      </form>
                      <form onSubmit={vhandleSubmit(handleCreateVenor)} className='w-3/12 '>
                        ...
                        <div className='flex-col flex items-center'>
                          ...
                          <div className="fieldset w-11/12 mt-4">
                            <button className="button w-full py-3 cursor-pointer bg-amber-100 text-black text-xs rounded-sm font-bold">{vendorLoading? <span className="loading loading-spinner loading-xs"></span>:"SUBMIT"}</button>
                          </div>
                        </div>
                      </form>
                      <form onSubmit={chandleSubmit(handleCreateContact)} className='w-6/12'>
                        <p className='text-sm font-bold text-black '>CONTACT CREATE</p>
                        ...
                        <div className="fieldset w-full mt-4">
                          <button className="button w-full py-3 cursor-pointer bg-amber-100 text-black text-xs rounded-sm font-bold">{contactLoading? <span className="loading loading-spinner loading-xs"></span>:"SUBMIT"}</button>
                        </div>
                      </form>
                  ...
                  <div className="collapse  bg-base-100 border border-base-300">
                  <input type="checkbox" name="my-accordion-3" checked={secondCheck} onChange={()=>setSecondCheck(true)} />
                  ...
                  <div className="collapse-content text-sm">
                      <div className='flex items-center flex-wrap'>
                        ...
                        <div className="fieldset w-3/12">
                          <legend className="fieldset-legend">Mail Address</legend>
                          <p className="input w-11/12 bg-[#e2e2e2]">{showSelectedContact?.mail}</p>
                        </div>
                        <div className="fieldset w-3/12">
                          <legend className="fieldset-legend">Phone Number</legend>
                          <p className="input w-11/12 bg-[#e2e2e2]">{showSelectedContact?.number}</p>
                        </div>
